...
public class GCrawl implements IGCrawl {
    ...
    public GCrawl(Player Player) {
        ...
        lic = new Listener() {
            ...
            public void PTogSE(PlayerToggleSneakEvent e) {
                if(e.getPlayer() == p && e.isSneaking()) SMP.getInstance().getCrawlManager().stopCrawl(GCrawl.this, GetUpReason.GET_UP);
            }
        };
    }
    ...
    public void start() {
        p.setSwimming(true);
        Bukkit.getPluginManager().registerEvents(li, SMP.getInstance());
        new BukkitRunnable() {
            @Override
            public void run() {

                Bukkit.getPluginManager().registerEvents(lim, SMP.getInstance());
                if(SMP.getInstance().C_GET_UP_SNEAK) Bukkit.getPluginManager().registerEvents(lic, SMP.getInstance());
                tick(p.getLocation());

            }
        }.runTaskLaterAsynchronously(SMP.getInstance(), 1);
    }
    ...
    private boolean checkCrawlValid() {
        if(cp.isInWater() || p.isFlying()) {
            new BukkitRunnable() {
                @Override
                public void run() {
                    SMP.getInstance().getCrawlManager().stopCrawl(GCrawl.this, GetUpReason.ACTION);
                }
            }.runTask(SMP.getInstance());
            return false;
        }
